...
const TWO_LINE_FUNC: ThemeFunc = (photo: Photo, input: ThemeOptionInput, store: Store, registry?: ElementRegistry) => {
  ...
  placeText(registry, context, 'topLabel', 'Top label', TOP_LABEL, W / 2, PADDING_TOP / 2);

  context.textAlign = TEXT_ALIGN;
  const text1 = applyTemplate(TEMPLATE1, photo, store, DIVIDER);
  placeText(
    registry,
    context,
    'line1',
    'Line 1',
    text1,
    TEXT_ALIGN === 'left' ? PADDING_LEFT : TEXT_ALIGN === 'center' ? W / 2 : W - PADDING_RIGHT,
    H - PADDING_BOTTOM / 2 - FONT_SIZE / 1.5
  );

  if (!store.disableExposureMeter) {
    const text2 = applyTemplate(TEMPLATE2, photo, store, DIVIDER);
    placeText(
      registry,
      context,
      'line2',
